...
- `app/(app)/` — retained logged-in author routes for `/create`, `/dashboard`,
  and `/settings` (has Navbar). `app/(marketing)/` owns Home, while
  `app/(site)/` owns reader routes including `/blog`. Parens are not part of
  the URL.
