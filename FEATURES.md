...
replaces the duplicated hotlinked fallback with the shared blank cover fallback
on cards and the reader; a missing cover renders a blank transparent surface
with a subtle bottom border. Adopting the same component in Review is pending in
the frozen-Review batch.
